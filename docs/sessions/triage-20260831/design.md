...
本仓风险等级 **personal**。infra 例外**逐增量判定，不整批套用**：

- **增量 1 不适用 infra 例外**，按 personal 档「连续 1 轮无新增 P1」收敛。
  它的改动核心是公开契约的字段透传——在失败路径上只**读取并记录**已有的值，
  不改变任何判定、分支或副作用时机。第 1 轮 Codex 独立审查 0 P1 / 0 P2 / 0 P3，
  已收敛（verdict 见 `reviews/increment1-verdict.md`）。
- **增量 2 适用 infra 例外**，走上一档「连续 2 轮无新增 P1」。它的改动核心**就是**
  失败路径与资源账本：副作用时机迁移 + 状态载体重构。

两档 review 轮次上限均为 3。

此处更正了本文件初稿「本批整体按 infra 例外」的写法：那句话把触发条件按批次套用，
而 `CLAUDE.md` 的原文条件是「**改动核心**落在失败路径 / provider 能力表」——
条件挂在改动本身，不挂在批次。整批套用会让纯增量卡也多烧一轮审查额度。
