...
> 已是成熟态（0.9.0 / 1023 测试 / mypy strict），不在本路线图范围内。
...
- **当前激活里程碑**：**无（待定）**。M1/M2 已完成，M3 已于 2026-08-06 降为 backlog
  （测量证伪痛点，见下），M4 待重新评估。**推进任何新工作前应先跑一次路线图审计五问。**
...
- **当前范围**：交付 L1/L2 契约，不交付语言 SDK；本次 Bun/TS runner 是基于 v0.9.0 契约的后续文档参考，不追溯进入已发布 tag。
- **关键决策**：
...
### M3-阶段0：就地修消费者 + 测量（**已完成**）

- **状态**：**已完成**（2026-08-06，AdCommentsPlatform PR #40）
- **测量结果（决定性）**：

  | 指标 | 值 |
  |---|---|
  | 样本量 | 300 条真实历史评论（Supabase 查 1000 取最近 300 条非空 content） |
  | 模型 | `gpt-4.1-mini` |
  | `no_json_match` | **0** |
  | `json_parse_error` | **0** |
  | `schema_mismatch` | **0** |
  | **JSON 提取总失败率** | **0 / 300 = 0%** |
  | 调用层失败（gateway/timeout/parse/empty） | **全部 0** |
...
- **顺带交付**：三条失败路径（`no_json_match`/`json_parse_error`/`schema_mismatch`）
  现在都有日志留痕，`client.ts` 的 `empty` 分支也补了日志。以后真出问题看得见——
  这是本阶段独立于 M3 去留的净收益。
- **已知局限（Codex finding #3 提出，如实保留）**：离线重放测的是「当前模型 + 当前网关」
  的新结果，不等于历史生产失败率。但结合调用层失败同样为 0，方向性结论足够强。
...
- **状态**：**BACKLOG（2026-08-06 按判据降级，不再是激活里程碑）**

  阶段 0 测出 JSON 提取失败率 **0/300 = 0%**，判据阈值是 > 2%。
  加上此前已证伪的「AdComments 没有通用重试」（SDK 默认 `maxRetries=2`），
  **shim 的两条价值论据现已全部归零，且调用层失败同样为 0**。

  这不是「暂缓」，是「当前证据不支持建它」。设计选型论证（加语言边际成本为 0、
  一份逻辑 N 种消费形态）仍然成立，但那是在**有第二个消费者**的前提下才兑现的价值。

  **复活条件**：出现第二个非 Python 消费者。届时**必须先解决下方三条架构级阻塞**，
  且需重新评估——因为那时最优解未必仍是 shim。
...
- **复活条件**：出现第二个非 Python 消费者，或阶段 0 证明就地修补不掉问题。
  （2026-08-06 更新：阶段 0 已完成，结果是**修不掉的问题根本不存在**——失败率本来就是 0。）

### 2026-08-06 第二次路线图审计（M3-阶段0 完成后）

- **里程碑真完成了吗？**：M3-阶段0 完成，有硬数字（0/300）。M1/M2 早已完成。
- **下一个目标还是对的吗？**：**不对，M3 已降 backlog**。判据预先定死、结果远低于阈值，
  按规则执行，无解释空间。
- **有没有漏掉的里程碑？**：**M2 的入口层证据仍然悬空且现在无解**——原本指望
  D3.2（360 向量驱动 shim 自测）兑现，但 Codex finding #14 指出那不构成独立实现交叉验证，
  且 shim 现在也不做了。这是一笔明确的欠账，需要单独想办法或接受它悬空。
- **新证据是否改变了工作顺序？**：是。M4（T5 来源元数据 + 分发）原本部分服务于 shim 场景，
  现在 T5（caps 数据新鲜度告警）仍独立有价值，T9（版本化分发）的紧迫性进一步下降——
  当前跨语言消费者数量为 0（AdComments 用的是自己的 73 行硬表，且工作良好）。
- **done 的定义还成立吗？**：M3 的 done 定义已作废（不做了）。M4 需要重定范围。
- **审计结论**：**当前无激活里程碑。** 本项目的「跨语言消费」这条线在证据层面已经收敛：
  唯一的非 Python 消费者不需要这个库提供的额外能力。继续投入前需要新的需求信号
  （第二个消费者，或现有消费者出现 caps 表维护痛点）。
...
`0.9.0` 已发布（`pyproject.toml` 同版本）并含 M2 JSON/strict 契约；本次 Bun/TS runner 是基于该契约的后续文档参考，不追溯进入已发布 tag。
